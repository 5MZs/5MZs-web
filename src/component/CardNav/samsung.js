...
const Samsung = () => {  // 컴포넌트 이름을 대문자로 시작하도록 변경
  const [samsungValue, setSamsungValues] = useState([]); // 변수 이름을 일관되게 유지
  ...
  const fetchDepartment = async () => {
    try {
      ...
      const response = await fetch(
        'http://15.165.135.177:3000/api/v1/cardbenefitsinfo/SAMSUNG'
      );
      ...
      setSamsungValues(json || []); // `json`이 실제 데이터 배열인지 확인
    } catch (error) {
  ...
  useEffect(() => {
    console.log('SamsungValue:', samsungValue); // 데이터가 제대로 업데이트되었는지 확인
  }, [samsungValue]);
  ...
  const goToLeft = () => {
    setCurrentIndex(
      (prevIndex) => (prevIndex - 1 + samsungValue.length) % samsungValue.length
    );
  };

  const goToRight = () => {
    setCurrentIndex((prevIndex) => (prevIndex + 1) % samsungValue.length);
  };
