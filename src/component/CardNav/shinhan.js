...
const Shinhan = () => {  // 컴포넌트 이름을 대문자로 시작하도록 변경
  const [shinhanValue, setShinhanValues] = useState([]); // 변수 이름을 일관되게 유지
  ...
  const fetchDepartment = async () => {
    try {
      ...
      setShinhanValues(json || []); // `json`이 실제 데이터 배열인지 확인
    } catch (error) {
  ...
  useEffect(() => {
    console.log('ShinhanValue:', shinhanValue); // 데이터가 제대로 업데이트되었는지 확인
  }, [shinhanValue]);
  ...
  const goToLeft = () => {
    setCurrentIndex(
      (prevIndex) => (prevIndex - 1 + shinhanValue.length) % shinhanValue.length
    );
  };

  const goToRight = () => {
    setCurrentIndex((prevIndex) => (prevIndex + 1) % shinhanValue.length);
  };
